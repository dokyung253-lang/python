# 넘파이 : 파이썬 언어 기반으로 수치계산 특화된 라이브러리(AI)
# 파이썬에 *리스트*가 존재하는데 굳이 *넘파이배열* 사용하는 이유? 수치관련 기능 차이
...
print( np.zeros( (3, 4) ) )     # 1. 모든 요소가 0으로 채워진 3행 4열의 배열
print( np.ones( (2, 5) ) )      # 2. 모든 요소가 1로 채워진 2행 5열의 배열
print( np.full( 3, 3 ) , 9 )    # 3. 모든 요소가 9로 채워진 3행 3열의 배열 (힌트: np.full)



# 문제 3: 연속된 수와 균등 간격 배열
print( np.arange( 10, 50+1, 5 ) )    # 1. np.arange를 사용하여 10부터 50까지 5씩 증가하는 배열을 생성하시오.
print( np.linspace( 0, 10, 5 ) )# 2. np.linspace를 사용하여 0부터 10 사이를 균등하게 5개의 구간으로 나눈 배열을 생성하시오.


# 문제 4: shape와 size 확인
# 배열의 모양(행, 열 크기)과 전체 요소의 개수(size)를 출력하는 코드를 작성하시오.
x = np.array([[1, 2, 3], [4, 5, 6]])
print( x.shape )     #(2, 3)     # shape (행개수, 열개수)
print( x.size )      # 6         # size 배열 내 전체 요소 수 반환
...
print(x.dtype)      # int32
x = np.array([True, False, True], dtype= np.bool_ )     # 파이썬과 _넘파이 타입 구분 : _(언더바)차이 
print(x.dtype)      # bool
...
print( x.dtype )        # <U6 : 유니코드, 잘못된 값 아님
y = x.astype( np.bytes_ )
print(y.dtype)          # |S6