...
data['Score'] = [85, 90, 95]
print(data)
data.loc[ data['Score'] >= 90, 'Name' ] = 'MVP'     # x.loc[조건식, 새로운/수정할열 ] = 새로운 값  # 90점 이상이면 'MVP'로 변환
print(data)
...
print( '문제 7', data.sort_values( by =['Age', 'Score' ] , ascending= [True, False] ) )
...
print( data['Fruit'].value_counts() )  # 고유값(중복수) 별 빈도수
data.columns = ['Item' , 'Style'] # data.columns 모든열, # data.indexs 모든행 # 전체수정 가능 # 원본수정
print( data )
data.rename( columns={ 'Fruit' : 'Item', 'Color' : 'Style' } )  # 원본수정 X
print( result )