...
y = [10, 20, 15, 25, 30]
y2 = [5, 15, 10, 20, 25]
plt.plot( x, y , c = 'blue' , linestyle = ':' , label = '매출')
plt.plot( x, y2, c = 'red' , linestyle = '-' , label = '비용')
plt.legend()
plt.grid( )
plt.title('월별 실적 분석')
plt.show()
...
국어 = [80, 90, 70]
수학 = [85, 85, 95]
x = np.arange( len(카테고리))
plt.bar( x, 국어 , width= 0.4 , label = '국어'  )
plt.bar( x , 수학 , width = 0.4, label = '수학')
plt.xticks( x, 카테고리 )
plt.ylabel( '점수' )
plt.show() 
...
sizes = [45, 25, 20, 10]
colors = ['beige', 'red', 'yellow' , 'lightgray']
explode = [0.1 , 0, 0, 0]
...
# 1.  데이터를 사용하여 파란색 점, 크기 100의 산점도를 그리시오.
# 2.  데이터를 사용하여 빈(bins)의 개수가 5인 
...
plt.hist( data, color = 'skyblue' ,  bins = 60 )
plt.show()
...
# 5행 4열의 리스트 데이터 [[50, 60, 70, 80], [10, 20, 30, 40], [90, 80, 70, 60], [30, 40, 50, 60],
# [20, 10, 5, 0]]를
