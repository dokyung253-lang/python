...
# (1) 두 개의 csv 파일 호출
data1 = pd.read_csv( './NPMPractice/서울특별시 공공자전거 대여이력 정보_2501.csv', encoding='cp949')
data2 = pd.read_csv( './NPMPractice/서울특별시 공공자전거 대여이력 정보_2502.csv', encoding='cp949')
...
# (2) 판다스 병합, .concat( [df1, df2], ignore_index= 새로운 인덱스 사용 여부)
df = pd.concat( [data1, data2], ignore_index= True)
# (3) 확인
df.info()
print( df.head() )  
# (4) 결측치 확인
print( df.isnull().sum() )  # 성별결측치 : 87085
# (5) 치환, df.replace( {기존값1, 새로운값1, 기존값, 새로운값2} )
# \n : 제어문자, \\n : \n 출력
df = df.replace( {'\\N' : None, '' : None} )
# (6) 결측치를 U로 보정, .fillna( 보정값 ), upper()대문자, lower()소문자
df['성별'].str.upper().fillna('U').fillna('U')  # U : 성별 알수없음
print( df.isnull().sum() ) # 결측 재확인 # 성별결측치 : 0
# (7) 수치형 문자형 구분하기 위해 df.info() 결과 확인, 또는 공식문자 참고
# 대여 대여소번호, 대여거치대, 이용시간(분), 이용거리(M)
# 만약, .to_numeric 변환 시 오류 발생하면 결측치로 수정, errors = 'coerce'
...
df['대여 시간대'] = df['대여일시'].dt.hour      # 대여 시간대 = 대여일시에서 '시'만 꺼내옴
df.info()   # 전처리 결과 확인  # 타입 변경됐는 지 확인
...
data1 = sns.countplot(data=df, x = '성별')
plt.show()

# 5-2 (시간대 분석): sns.barplot을 사용하여 24시간 중 어느 시간대에 대여가 집중되는지 분석한다.
# .barplot(x축(시간대) , y축 : 해당 시간대 대여 갯수 )
...
# corr() : 변수들 간의 상관계수( -1 ~ 1 )를 반환
# df.select_dtypes( include = '선택할타입' ) : 데이터타입으로 데이터 선택
number_data = df.select_dtypes( include = 'number' ) # 자동으로 수치 데이터열만 추출
