...
# .concatenate( (x,y), axis = 0 ) , axis = 0(행 기준) 1(열 기준)
x = np.array([[1,2], [3,4]])
y = np.array([[5,6], [7,8]])
...
print( np.sort( x, axis = None ) )  # [1 2 7 8 9 12]    # 1차원 평탄화 후 정렬
# 2차원 정렬 내림차순 주의할 점 : 2차원 슬라이싱 사용, [행슬라이싱, 열슬라이싱]
print( np.sort( x, axis = 0)[ :: -1 , : ] )      # 열 기준 내림차순
print( np.sort( x, axis = 1) [ :: -1 ])     # 행 기준 내림차순

# np.sort() 복사본 반환 vs 배열.sort() 원본 수정
x = np.array( [5, 1, 3] )
print( np.sort(x))      # [1 3 5] , 복사본
print( x.sort() )       
print( x )              # [1 3 5] , 수정된 원본

# .lexsort( , 1차기준 ), 다중정렬, 1차 정렬 후 만약에 1차정렬에서 동일한 값이 존재하면 동일한 값까지 
x = np.array( [25, 30, 22, 24 ])    # 1차원 리스트
y = np.array( ['영숙', '상철', '콩순', '진감'] ) # 1차원 리스트
z = np.lexsort( ( x, y ) ) 
print( x[z] )   # y 먼저 정렬 후 ,[30 25 24 22]
print( y[z] )   # y 정렬 후 동일한 값끼리 2차 정렬 , ['상철' '영숙' '진감' '콩순']
...
print( np.where( x > 30 , x , 0 ) )         # x가 30보다 크면, x 반환, 아니면 0 반환
print( np.where( y % 2 == 0 , y, 1 ) )      # 만약에 요소값이 짝수이면 그대로, 아니면 1 반환

# 마스크 필터링, 마스크(가리다) 조건식에 False만 사용 , .ma.array( x, mask = 조건식 )
con1 = x > 30       # 30 이상 가린다.
