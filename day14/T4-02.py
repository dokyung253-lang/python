...
df = pd.read_csv( 
    ...
    encoding='utf-8',                                # 인코딩( 한글: utf-8 , cp949 , enc-kr ) , 파일마다 다를 수 있다.     
...
# 2. .excel 파일 불러오기(pip install openpyxl 설치)
df = pd.read_excel(
    './day14/data.xlsx',
...
# 4. .xml 파일 불러오기 (pip install lxml 설치 )
...
df.to_json( 
    ...
    date_format='iso'            # 날짜형식을 표준'YYYY-MM-DD'방식 지정
   )
# 4. .xml 내보내기
