# 객체 : 속성(상태)과 메소드(행위)로 이루어진 추상화된 개념
# 클래스: 객체를 프로그래밍에서 표현하기 위한 설계도
# 인스턴스 : 클래스 기반으로 생성한 객체
# 객체와 인스턴스의 차이 : 객체는 논리/개념적/추상적/설계 인스턴스 실제데이터/db저장/물리적
# 생성자 : 인스턴스가 생성될 때 실행되는 함수 = 초기화역할 함수
# p.472 참조

# [1] 클래스 만들기
class Student :
    # [2] 생성자 선언
    def __init__(self, name, korean, math, english, science):   # 언더바 앞뒤로 2개씩
        # self : 자기자신 
    ...
    def get_sum(self):      # 해당메소드를 호출한 인스턴스 가리킴, PY : self vs JAVA : this
...
# [5] 인스턴스 내 속성값 호출 , 인스턴스.변수명, .(도트/연산자)
...
Student = [ {'name':"윤인성5", "korean": 87, 'math': 98, 'english': 88, 'science': 95},
            {'name':"윤인성5", "korean": 87, 'math': 98, 'english': 88, 'science': 95}
          ]     # 설계
