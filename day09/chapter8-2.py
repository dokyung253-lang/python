...
class Student :
    ...
    # cls(클래스): 설계도 하나 vs self(인스턴스): 설계도 기반으로 만들어진 여러개의 세부설계들
    # 클래스함수 @classmethod vs JAVA : static
    @classmethod        # 데코레이터
    def print( cls ):
        print("클래스 함수 호출")
        print( cls.count )
...
circle = Circle(10)     # 변수에 저장
# print( circle.radius )  # 프라이빗 변수이므로 직접 호출 시 오류 발생
...
print( child.value ) # 자식은 부모의 멤버변수와 멤버함수를 사용(호출)할 수 있다. <물려받음>

# [10] 상속 이용한 나만의예외클래스 만들기
class CustomException(Exception) : # Exception 클래스로 부터 상속받는다.
